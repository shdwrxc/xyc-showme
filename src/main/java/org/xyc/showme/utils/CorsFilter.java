...
public final class CorsFilter implements Filter {
    ...
    public void init(final FilterConfig filterConfig) throws ServletException {
        // Initialize defaults
        parseAndStore(DEFAULT_ALLOWED_ORIGINS, DEFAULT_ALLOWED_HTTP_METHODS, DEFAULT_ALLOWED_HTTP_HEADERS, DEFAULT_EXPOSED_HEADERS, DEFAULT_SUPPORTS_CREDENTIALS, DEFAULT_PREFLIGHT_MAXAGE,
    ...
    private boolean isOriginAllowed(final String origin) {
        ...
        return allowedOrigins.contains(origin) || isSegmentAllowed(origin);
    }

    /**
     *
     * checks if the origin is allowed by segment
     *
     * @param origin
     * @return <code>true</code> if origin is allowed; <code>false</code>
     * otherwise.
     */
    private boolean isSegmentAllowed(String origin) {
        int i = origin.indexOf("//");
        if (i == -1) {
            return false;
        }
        for (String e : allowedOrigins) {
            if (origin.startsWith(e) || origin.substring(i + 2).startsWith(e)) {
                return true;
            }
        }
        return false;
     ...
     * @throws ServletException
     */
    private void parseAndStore(final String allowedOrigins, final String allowedHttpMethods, final String allowedHttpHeaders, final String exposedHeaders, final String supportsCredentials,
